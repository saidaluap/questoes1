...
db.all(
    ...
    (err, rows) => {
      ...
      db.get(
        'SELECT COUNT(DISTINCT questao_id) as total FROM historico_respostas WHERE user_id = ?',
        [userId],
        (err2, countResult) => {
          if (err2) {
            return res.status(500).json({ error: 'Erro ao contar respostas' });
          }
...
app.post('/api/historico/salvar-resposta', authenticateToken, (req, res) => {
  ...
  db.run(
    `INSERT INTO historico_respostas
      (user_id, questao_id, resposta_usuario, resposta_correta, acertou, data_resposta)
