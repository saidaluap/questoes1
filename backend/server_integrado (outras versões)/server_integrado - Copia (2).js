...
app.get("/api/questoes", authenticateToken, (req, res) => {
  ...
  const page = parseInt(req.query.page) || 1;
  const limit = 5; // 5 questões por página conforme solicitado
  const offset = (page - 1) * limit;
...
// Buscar o histórico de respostas do usuário logado
app.get('/api/historico', authenticateToken, (req, res) => {
  const userId = req.user.id;
  // Sua consulta ao banco para buscar respostas do userId
  db.all('SELECT * FROM historico_respostas WHERE usuario_id = ?', [userId], (err, rows) => {
    if (err) {
...
// Buscar estatísticas do histórico do usuário
app.get('/api/historico/estatisticas', authenticateToken, (req, res) => {
  const userId = req.user.id;
  // Consulta para gerar estatísticas do usuário
  db.get('SELECT COUNT(*) as total_respostas, SUM(correto) as total_acertos FROM historico_respostas WHERE usuario_id = ?', [userId], (err, row) => {
    if (err) {
...
app.post('/api/historico/salvar-resposta', authenticateToken, (req, res) => {
  const userId = req.user.id;
  const { questaoId, respostaSelecionada, correto } = req.body;
  if (!questaoId || respostaSelecionada == null || correto == null) {
    return res.status(400).json({ error: 'Dados incompletos para salvar resposta' });
  }
  db.run('INSERT INTO historico_respostas (usuario_id, questao_id, resposta_selecionada, correto, data_resposta) VALUES (?, ?, ?, ?, CURRENT_TIMESTAMP)',
    [userId, questaoId, respostaSelecionada, correto], function(err) {
      if (err) {
